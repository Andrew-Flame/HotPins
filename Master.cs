...
namespace HotPins {
    ...
    public class Master : BaseUnityPlugin {
        ...
        private Dictionary<KeyCode[], Pin.Pin> keyBundles = new Dictionary<KeyCode[], Pin.Pin>();
        #endregion
        ...
        void Awake() {
            ...
            /* Sorting binds (we need the first keyboard shortcuts in the dictionary with the largest number of keys involved) */
            {
                ...
                foreach (KeyValuePair<KeyCode[], Pin.Pin> bundle in keyBundles)  //Getting the maximum number
                ...
                while (tmpDictionary.Count != keyBundles.Count) {  //Until both dictionaries have the same number of entries
                    foreach (KeyValuePair<KeyCode[], Pin.Pin> bundle in keyBundles)  //Adding entries from the "longest" to the "shortest"
        ...
        void Update() {
            ...
            if (Input.GetKeyDown(autoPinKey)) {
                ...
                foreach (Transform proxyLocation in GameLocationProxy.locationsProxy) {
                    ...
                    if (linearDistance < autoPinRadius) {  //If the location is close
                        ...
                        if (pinName != string.Empty) Pin.Add.Run(new Pin.Pin(autoPinType, pinName), proxyLocation.position);  //If it doesn't empty, create a pin
                    }
                }
            ...
            foreach (KeyValuePair<KeyCode[], Pin.Pin> bundle in keyBundles) {  //Checking whether any of the custom keys are pressed
